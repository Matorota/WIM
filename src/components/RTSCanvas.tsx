...
export const RTSCanvas: React.FC<RTSCanvasProps> = ({ width, height }) => {
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    if (isMouseDown) {
      if (e.buttons === 4) { // Middle mouse for panning
    ...
    if (selectionBox && e.button === 0) {
      ...
      if (Math.abs(maxX - minX) < 1 && Math.abs(maxY - minY) < 1) {
        ...
        const clickedBuilding = buildings.find(building => {
          const inBounds = start.x >= building.position.x && 
  ...
  }, [selectionBox, screenToWorld, units, buildings, currentPlayerId, selectUnits, selectBuildings, clearSelection]);
  ...
  useEffect(() => {
    ...
    buildings.forEach(building => {
      ...
      // Building body
      ctx.fillStyle = building.isConstructed ? player.color : 'rgba(128,128,128,0.5)';
      ctx.fillRect(
        building.position.x,
        building.position.y,
        building.size.width,
      ...
      if (!building.isConstructed) {
        const progressHeight = (building.size.height * building.constructionProgress) / 100;
      ...
      if (building.rallyPoint) {
        ...
        ctx.fillStyle = player.color;
        ctx.beginPath();
        ctx.arc(building.rallyPoint.x, building.rallyPoint.y, 0.5, 0, 2 * Math.PI);
        ctx.fill();
      }
    });
    ...
    units.forEach(unit => {
      ...
      ctx.textAlign = 'center';
      ctx.fillText(typeSymbol, unit.position.x, unit.position.y + 2 / camera.zoom);
      ...
      if (unit.carryingResources) {
        ...
        ctx.beginPath();
        ctx.arc(unit.position.x + 0.5, unit.position.y - 0.5, 0.3, 0, 2 * Math.PI);
        ctx.fill();
      }
    });
    ...
    width, height, camera, mapSize, resourceNodes, buildings, units, players,
    selectedUnits, selectedBuildings, selectionBox, currentPlayerId
  ]);
