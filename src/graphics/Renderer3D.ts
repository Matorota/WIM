...
export class Renderer3D {
  ...
  private getPlayerColor(playerId: string): string {
    ...
    switch (playerId) {
      case "player":
        return "#4A90E2"; // Blue for human player
      case "ai":
        return "#E24A4A"; // Red for AI player
      case "ai1":
        return "#E24A4A"; // Red for AI player 1
      case "ai2":
        return "#FFA500"; // Orange for AI player 2
      case "ai3":
        return "#9B59B6"; // Purple for AI player 3
      default:
        return "#808080"; // Gray for unknown players
    }
  }
  ...
  private getUnitColor(playerId: string): string {
    ...
    switch (playerId) {
      case "player":
        return "#2E8B57"; // Green for human player units
      case "ai":
        return "#CD5C5C"; // Red for AI units
      case "ai1":
        return "#CD5C5C"; // Red for AI player 1 units
      case "ai2":
        return "#FF8C00"; // Dark orange for AI player 2 units
      case "ai3":
        return "#8E44AD"; // Dark purple for AI player 3 units
      default:
        return "#696969"; // Dark gray for unknown players
    }
  }
  ...
  private buildRenderQueue(units: Unit[], buildings: Building[]): void {
    ...
    buildings.forEach((building) => {
      this.renderQueue.push({
        ...
        position: {
          ...
          z: this.getBuildingHeight(building.type),
        },
    ...
    units.forEach((unit) => {
      ...
      const bobOffset =
        Math.sin(this.animationTime * 0.003 + unit.position.x) * 0.1;
  ...
  private calculateDepth(position: {
    x: number;
    y: number;
    z: number;
  }): number {
  ...
  private project3DToScreen(pos3D: { x: number; y: number; z: number }): {
    x: number;
    y: number;
    scale: number;
  } {
    ...
    const screenX =
      (isoX - this.camera.position.x + 25) * this.camera.zoom * 12 +
      this.canvas.width / 2;
    const screenY =
      (isoY - this.camera.position.y + 25) * this.camera.zoom * 8 +
  ...
  private renderBuilding(
    obj: RenderObject3D,
    screenPos: { x: number; y: number; scale: number }
  ): void {
  ...
  private renderUnit(
    ...
    switch (obj.model) {
      ...
      case "helicopter":
        ...
        const rotor1X =
          screenPos.x + Math.cos(this.animationTime * 0.02) * size;
  ...
  private renderTerrain(
    ...
    if (_obj.position.z > 0.2) {
      ...
      this.ctx.fillStyle = "rgba(0, 0, 0, 0.2)";
      this.ctx.fillRect(
        screenPos.x - size / 2 + 1,
        screenPos.y - size / 2 + 1,
        size - 2,
        size - 2
      );
    }
  ...
  private renderMinimap(units: Unit[], buildings: Building[]): void {
    ...
    buildings.forEach((building) => {
      this.ctx.fillStyle =
        building.playerId === "player" ? "#4A90E2" : "#E24A4A";
      this.ctx.fillRect(
